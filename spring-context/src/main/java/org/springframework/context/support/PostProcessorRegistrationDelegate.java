...
class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			//把 IOC 容器 强制转为 BeanDefinitionRegistry 类型的
			BeanDefinitionRegistry registry = (BeanDefinitionRegistry) beanFactory;
			//创建一个普通的 BeanFactoryPostProcessor的 list 的组件
			List<BeanFactoryPostProcessor> regularPostProcessors = new LinkedList<BeanFactoryPostProcessor>();
			//创建一个 BeanDefinitionRegistryPostProcessor 类型的 list
			List<BeanDefinitionRegistryPostProcessor> registryProcessors = new LinkedList<BeanDefinitionRegistryPostProcessor>();
			//处理容器硬编码(new 出来的)带入的 beanFacotryPostProcessors
			for (BeanFactoryPostProcessor postProcessor : beanFactoryPostProcessors) {
				//判断是不是 BeanDefinitionRegistryPostProcessor
				...
				else {//判断不是 BeanFactoryPostProcessor
					//加入到集合中
			...
			//创建一个当前注册的 BeanDefinitionRegistryPostProcessor 的集合
			List<BeanDefinitionRegistryPostProcessor> currentRegistryProcessors = new ArrayList<BeanDefinitionRegistryPostProcessor>();

			// First, invoke the BeanDefinitionRegistryPostProcessors that implement PriorityOrdered.
			//第一步:去容器中查询是否有 BeanDefinitionRegistryPostProcessor 类型的,调用postProcessBeanDefinitionRegistry 方法--实现了 PriorityOrdered 接口的
			...
			//第三步:调用普通的 BeanDefinitionRegistryPostProcessors 没用实现 PriorithOrdered 和 Ordered 接口的postProcessBeanDefinitionRegistry 方法
			...
			//第四步:调用上述所有实现了 BeanDefinitionRegistryPostProcessor 的接口,调用postProcessBeanFactory（）方法
		...
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
		//第五步:去 IOC 容器中获取 BeanFactoryPostProcessor 类型的，分离 实现了 PriorityOrdered 接口的、Ordered 接口的、普通的
		String[] postProcessorNames =
				beanFactory.getBeanNamesForType(BeanFactoryPostProcessor.class, true, false);

		// Separate between BeanFactoryPostProcessors that implement PriorityOrdered,
		// Ordered, and the rest.
		//分离实现了 PriorityOrdered 接口的 Ordered 接口的 普通的
		...
		//第六步:调用实现PriorityOrdered接口的BeanFactoryPostProcessor的postProcessBeanFactory()方法
		sortPostProcessors(priorityOrderedPostProcessors, beanFactory);
		invokeBeanFactoryPostProcessors(priorityOrderedPostProcessors, beanFactory);

		// Next, invoke the BeanFactoryPostProcessors that implement Ordered.
		//第七步:调用实现Ordered接口的BeanFactoryPostProcessor的postProcessBeanFactory()方法
		...
		//第八步:调用普通的没有实现PriorityOrdered接口和Ordered接口的BeanFactoryPostProcessor的postProcessBeanFactory()方法
	...
	private static void invokeBeanDefinitionRegistryPostProcessors(
		...
		for (BeanDefinitionRegistryPostProcessor postProcessor : postProcessors) {
			//这里有个很重要的工厂后处理器---ConfigurationClassPostProcessor是一个BeanFactory的后置处理器，因此它的主要功能是参与BeanFactory的建造，在这个类中，会解析加了@Configuration的配置类，还会解析@ComponentScan、@ComponentScans注解扫描的包，以及解析@Import等注解。
